...
int main() {
    ...
    for (int i = 0; i < N_ELE / 2; i++) {
        ...
        expected[2 * i] = re1 * red - im1 * imd + re2;
        expected[2 * i + 1] = re1 * imd + im1 * red + re2; // Wait, im2!
        // Re-reading formula: im_new = re1*im_vd + im1*re_vd + im2
    }
    // Correcting formula in loop
    for (int i = 0; i < N_ELE / 2; i++) {
        T re1 = vs1[2 * i], im1 = vs1[2 * i + 1];
        T re2 = vs2[2 * i], im2 = vs2[2 * i + 1];
        T red = vd[2 * i], imd = vd[2 * i + 1];
        expected[2 * i] = re1 * red - im1 * imd + re2;
        expected[2 * i + 1] = re1 * imd + im1 * red + im2;
    }
    ...
    __asm__ volatile(
        ...
        "vmv.v.v v6, v0\n"      // v6 = re_d
        "vfmadd.vv v0, v2, v4\n"  // v0 = re1*re_d + re2
        "vfnmsac.vv v0, v3, v1\n" // v0 = re1*re_d + re2 - im1*im_d
        "vfmadd.vv v1, v2, v5\n"  // v1 = re1*im_d + im2
        "vfmacc.vv v1, v3, v6\n"  // v1 = re1*im_d + im2 + im1*re_d
        VST2_INS " v0, (%0)\n"
        :
