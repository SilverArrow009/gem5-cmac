...
main()
{
    ...
    for (int i = 0; i < N_ELE / 2; i++) {
        ...
        double vd_r = (double)vd[i * 2], vd_i = (double)vd[i * 2 + 1];
        // vcfmadd.vf vd, rs1, vs2 => vd = rs1 * vd + vs2
        expected[i * 2] = (T)(rs1 * vd_r + s2_r);
        expected[i * 2 + 1] = (T)(rs1 * vd_i + s2_i);
    }
    ...
    int pass = 1;
#ifdef DEBUG
    printf("vcfmadd.vf Test (VLEN=%d, ELEN=%d, N_ELE=%d)\n", VLEN, ELEN, (int)N_ELE);
    for (int i = 0; i < N_ELE; i++) {
