...
class CVA6_FloatALUFU(MinorFU):
    ...
    timings = [
        MinorFUTiming(
            description="Scalar Float compare",
            opClasses=minorMakeOpClassSet(["FloatCmp"]),
            srcRegsRelativeLats=[2],
            extraAssumedLat=2,
        ),
        MinorFUTiming(
            description="Scalar Float convert",
...
class Ara_SimdIntAluFU(MinorFU):
    ...
    def __init__(self, vlen, elen):
        ...
        reduction_penalty = int(math.log2(vlen / elen))
        self.timings = [
            MinorFUTiming(
                description="Vector Reductions (Sum)",
                # 6 (Transfer) + reduction_penalty
...
class Ara_SimdIntMulFU(MinorFU):
    ...
    timings = [
        MinorFUTiming(
            description="Vector Accumulations (MAC)",
            # 6 (Transfer) + 3 (Mult) + 1 (Add/Sub) = 10
            opClasses=minorMakeOpClassSet(["SimdAddAcc", "SimdMultAcc"]),
            srcRegsRelativeLats=[2],
            extraAssumedLat=1,
        ),
    ]
...
class Ara_SimdFloatALUFU(MinorFU):
    ...
    def __init__(self, vlen, elen):
        ...
        reduction_penalty = int(math.log2(vlen / elen)) * 4  # lat(fadd)=4
        self.timings = [
            MinorFUTiming(
                description="Vector Float comparison (FloatCmp)",
                # 6 (Transfer) + 3 = 9
                opClasses=minorMakeOpClassSet(["SimdFloatCmp"]),
                srcRegsRelativeLats=[2],
                extraAssumedLat=2,
            ),
            MinorFUTiming(
                description="Vector Float convert (FloatCvt)",
                # 6 (Transfer) + 2 = 8
                opClasses=minorMakeOpClassSet(["SimdFloatCmp"]),
                srcRegsRelativeLats=[2],
                extraAssumedLat=1,
            ...
            MinorFUTiming(
                description="Vector Float Reductions (VFSum)",
                # 6 (Transfer) + reduction_penalty
                opClasses=minorMakeOpClassSet(["SimdFloatReduceAdd"]),
                srcRegsRelativeLats=[2],
                extraAssumedLat=reduction_penalty - 1,
            ),
        ]
...
class Ara_SimdFloatMulFU(MinorFU):
    ...
    timings = [
        MinorFUTiming(
            description="Vector Float Accumulations (VFMAC)",
...
class Ara_SimdComplexFloatMulFU(MinorFU):
    """Handles all vector floating-point complex number multiplications and MAC operations."""

    # Base complex Float multiplicaiton (Assuming fully pipelined implementation) = 3 cycles.
    # Penalty = 2 (For in-Lane dispatcher) + 4 (Base Transfer delay) = 6 cycles base
    opClasses = minorMakeOpClassSet(["SimdComplexMult", "SimdComplexMultAcc"])
    opLat = 9
    issueLat = 1
    timings = [
        MinorFUTiming(
            description="Vector Float Accumulations (VFMAC)",
            # 6 (Transfer) + 3 (Mult) + 1 (Add/Sub) = 10
            opClasses=minorMakeOpClassSet(["SimdComplexMultAcc"]),
            srcRegsRelativeLats=[2],
            extraAssumedLat=1,
        )
    ]
...
class Ara_SimdFloatComplexDivFU(MinorFU):
    """Handles all vector floating-point complex division operations."""

    # Base complex Float division = 1 + 1 + 20 + 20 = 42 cycles.
    # Penalty = 2 (For in-Lane dispatcher) + 4 (Base Transfer delay) = 6 cycles base
    # Iterative Div (42) + 4 (Transfer) + 2 (ALU Penalty) = 48
    opClasses = minorMakeOpClassSet(["SimdComplexDiv"])
    opLat = 48
    issueLat = 48
...
class UnifiedCVA6AraFUPool(MinorFUPool):
    def __init__(self, vlen, elen):
        ...
        self.funcUnits = [
            ...
            CVA6_FloatDivFU(),
            CVA6_MiscFU(),
            Ara_SimdIntAluFU(vlen, elen),
            Ara_SimdIntMulFU(),
            Ara_SimdIntDivFU(),
            Ara_SimdFloatALUFU(vlen, elen),
            Ara_SimdFloatMulFU(),
            Ara_SimdFloatDivFU(),
            Ara_SimdComplexFloatMulFU(),
            Ara_SimdFloatComplexDivFU(),
            Ara_MemStridedFU(),
...
class HeterogeneousCore(BaseCPUCore):
    def __init__(self, cpu_id, vlen, elen):
        core = RiscvMinorCPU(cpu_id=cpu_id)
        core.executeFuncUnits = UnifiedCVA6AraFUPool(vlen, elen)
...
def run_simulation(binary_path, binary_opts, vlen, elen):
    cache_hierarchy = PrivateL1PrivateL2CacheHierarchy(
        l1d_size="32KiB", l1i_size="32KiB", l2_size="512KiB"
    )
    memory = SingleChannelDDR3_1600("1GiB")
    processor = BaseCPUProcessor(cores=[HeterogeneousCore(0, vlen, elen)])
    board = SimpleBoard(
        clk_freq="1GHz",
        processor=processor,
        memory=memory,
    ...
    if binary_opts is not None:
        board.set_se_binary_workload(
            BinaryResource(local_path=binary_path), arguments=binary_opts
        )
    else:
        board.set_se_binary_workload(BinaryResource(local_path=binary_path))
    simulator = Simulator(board=board)
    print(
        f"Beginning simulation with CVA6 + Ara custom model (VLEN={vlen}, ELEN={elen})!"
    )
    simulator.run()
...
if __name__ in ("__m5_main__"):
    ...
    parser.add_argument(
        "--bin-args", type=str, help="Arguments to the binary to run"
    )
    parser.add_argument(
        "--vlen", type=int, default=1024, help="Vector length in bits"
    )
    parser.add_argument(
        "--elen", type=int, default=64, help="Element width in bits"
    )
    ...
    if args.bin_args is not None:
        ...
        bin_opts = None
    run_simulation(args.binary, bin_opts, args.vlen, args.elen)
